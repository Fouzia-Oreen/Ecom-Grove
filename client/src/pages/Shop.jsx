...
const Shop = () => {
  ...
  const { categories, products, checked, radio, brands, subCategories } = useSelector(
    (state) => state.shop
  );
  ...
  }, [categoriesQuery.data, brandsQuery.data, subCategoriesQuery.data, dispatch]);
  ...
  const handleSubCategoryClick = (subCategories) => {
    const productsBySubCategory = filteredProductsQuery.data?.filter(
      (product) => product.subCategory === subCategories
    );

    dispatch(setProducts(productsBySubCategory));
  };
                  ...
                  <div key={subCategory._id} className="mb-2">
                    <div className="flex items-center mr-4">
                      <input
                        ...
                        className="focus:ring-color_6  focus:ring-[3px] rounded-full bg-color_6"
                        checked={checked && checked.includes(subCategory._id) }
                      />
